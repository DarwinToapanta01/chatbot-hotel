              ...
              <tr key={room.id} className="hover:bg-fog-gray transition-colors group">
                ...
                <td className="py-8 px-16 text-right">
                  <div className="flex items-center justify-end gap-8 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="text-slate-grille hover:text-deep-teal transition-colors" title="Ver Detalles">
                      <Eye className="w-14 h-14" />
                    </button>
                    {room.estado === 'DISPONIBLE' && (
                      <button className="text-slate-grille hover:text-spring-leaf transition-colors" title="Marcar Ocupada"
                        onClick={() => cambiarEstado(room.id, 'OCUPADA')}>
                        <CalendarPlus className="w-14 h-14" />
                      </button>
                    )}
                    <button className="text-slate-grille hover:text-amber-pop transition-colors" title="Editar">
                      <Edit2 className="w-14 h-14" />
                    </button>
                    {(room.estado === 'OCUPADA' || room.estado === 'DISPONIBLE') && (
                      <button className="text-slate-grille hover:text-electric-blue transition-colors" title="Marcar Limpieza"
